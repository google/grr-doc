...
To Encrypt and password the code & driver signing certificates
...
NOTE: You *must* use “des3”. Other algorithms do not work.

NOTE: There *must* be an extra line between the encrypted PEM header and the encoded key. The key is double-spaces and indented two spaces exactly like all other keys in configuration file. 
