...
Build the package using docker as below. This first command builds the docker image, which includes the deb, and the second command starts a container and copies out the file.

----
docker build -t build_package -f docker/Dockerfile.build_server_deb .
docker run --rm --entrypoint cat build_package \
  /usr/src/grr-server_3.1.0-2_amd64.deb > /tmp/grr-server_3.1.0-2_amd64.deb
----
