...
=== 3.1.0.2 to 3.2.0.0 (Aug 15 2017)
After the 3.2 release, we plan on releasing more frequently, since we have
automated a large part of the release process.
 ...
 - *WARN* Rekall flows are moved to 'DEBUG' category. They won’t be visible
   unless users enable 'Mode: DEBUG' in the settings menu (accessible via the
   settings button in top right corner of GRR AdminUI).
 - *WARN* A number of backwards-incompatible datastore schema changes were done.
   ...
   back up your current system in case this data is critical.
 ...
 - Build process for clients and server is significantly improved. Client
